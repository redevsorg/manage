...
export default function EditDocument({ collection }: { collection: string }) {
  ...
  useEffect(() => {
    window.history.replaceState({}, '', `/redevs/${collection}/${slug}`)
        ...
        <FormProvider {...methods}>
          ...
          >
            <form className="m-auto max-w-[700px] space-y-4">
              <DocumentTitleInput
                id="title"
                className="w-full resize-none outline-none bg-black text-5xl scrollbar-hide min-h-[55px] overflow-hidden"
